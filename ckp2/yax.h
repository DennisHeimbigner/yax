...
/* The tokenizer takes two inputs:
1. The lexical state 
2. A token state
...
/* effectively opaque */
typedef struct yax_state yax_state;

/*
 * Input is a null-terminated string of the whole xml text;
*/
extern yax_err yax_create(const char* input, int flags, yax_state**);
extern yax_err yax_free(yax_state*);

/* Allow for changing the flags after create; return previous flag set */
extern void yax_setflags(yax_state*,int flags);
extern int yax_getflags(yax_state*);

/* Obtain the next token; user provides the yax_token token state */

extern yax_err yax(yax_state*, yax_token*);
...
/*
  ...
  Caller must free returned string.
   A null return value implies out of memory.
*/
extern char* yax_trace(yax_state*, yax_token*);
