...
yylex(YYSTYPE *lvalp, Parser* parser)
{
	...
	case YAX_OPEN:
	    if(strcmp("number",tokenstate->name)==0)
		yytoken = NUMBEROPEN;
	    else if(strcmp("plus",tokenstate->name)==0)
		yytoken = PLUS;
	    else if(strcmp("minus",tokenstate->name)==0)
		yytoken = MINUS;
	    else
	...
	case YAX_ATTRIBUTE: /* ignore */
	    if(strcmp("value",tokenstate->name)==0)
	        yytoken = VALUEATTR;
	    else
		yytoken = ERROR;
	    break;
	...
	case YAX_TEXT: /* ignore if only whitespace */
	    if(!allwhitespace(tokenstate->text))
		yytoken = TEXT;
	    break;
...
xmlparse(char* input, Expr** ep)
{
    ...
    err = yax_create(input,YXFLAGS,&parser->lexstate);
    if(err) {
      fprintf(stderr,"%s\n",yax_errstring(err));
      exit(1);
    }
